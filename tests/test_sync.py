...
class TestPgIam(object):
    ...
    def setup_db(self, sync_db):
        """Assign sync_db fixture to self.db for all test methods."""
        self.db = sync_db
    ...
    def test_pgiam(self) -> None:
        ...
        try:
            ...
            assert len(capabilities[0][group_col_idx]) == 1 and capabilities[0][
                group_col_idx
            ] == [_in_group1]
            assert capabilities[1][name_col_idx] == "test2"
            assert len(capabilities[1][group_col_idx]) == 1 and capabilities[1][
                group_col_idx
            ] == [_in_group1]
            ...
            assert len(capabilities[2][group_col_idx]) == 2 and capabilities[2][
                group_col_idx
            ] == [_in_group1, "{0}-group".format(_in_uname)]
            ...
            g_req_attr_idx = 10
            assert len(gs) == 2
            assert gs[0][g_rank_idx] == 1 and gs[0][g_req_gr_idx] == [
                "self",
                "moderator",
            ]
            assert gs[1][g_rank_idx] == 1 and gs[1][g_req_gr_idx] == [
                _in_group3,
                _in_group4,
            ]
            ...
            print(
                self.db.capability_grant_rank_set(self.grant_id_from_name(grname3), 1)
            )
            gs = self.db.exec_sql(
